...
CHUNKSIZE = 512
ALLOWED_DATA_URI_SCHEMES = {"http", "https", "file"}
...
def validate_data_uri(data_uri: str) -> None:
    """
    Validate that the data URI uses an allowed scheme and matches
    ...
    if parsed.scheme not in ALLOWED_DATA_URI_SCHEMES:
...
def load_data(data_uri: str, dataset: SqlaTable, database: Database) -> None:
    ...
    data = request.urlopen(data_uri)  # pylint: disable=consider-using-with  # noqa: S310  # nosec B310 - scheme validated in validate_data_uri
