...
def validate_data_uri(data_uri: str) -> None:
    """
    Validate that the data URI uses an allowed URL scheme and is configured
    on DATASET_IMPORT_ALLOWED_URLS.
...
def load_data(data_uri: str, dataset: SqlaTable, database: Database) -> None:
    ...
    data = request.urlopen(data_uri)  # noqa: S310  # nosec B310  # pylint: disable=consider-using-with
